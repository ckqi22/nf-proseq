...
#
# Input: two count matrices produced by featurecounts_to_matrix.R, each with
#   columns: gene_id, length, <sample1>, <sample2>, ...
...
compute_pausing_index <- function(tss, gb, min_len) {
    ...
    if (colnames(gb)[1] != gene_col)
        stop("TSS and gene body matrices must share the same first column (gene ID)")
    ...
    genes   <- tss[[gene_col]]
    tss_len <- as.numeric(tss[["length"]])   # TSS window length (constant)
    gb_len  <- as.numeric(gb[["length"]])    # gene body length

    sample_cols <- setdiff(colnames(tss), c(gene_col, "length"))

    out <- data.frame(gene_id = genes, gene_length = gb_len, stringsAsFactors = FALSE)
    for (s in sample_cols) {
        pause <- as.numeric(tss[[s]])
        body  <- as.numeric(gb[[s]])
        pi    <- (pause / tss_len) / (body / gb_len)
        pi[body <= 0] <- NA_real_   # undefined when the gene body has no reads
        out[[paste0(s, "_Pause")]] <- pause
        out[[paste0(s, "_Body")]]  <- body
        out[[paste0(s, "_PI")]]    <- pi
    }
    out
}
