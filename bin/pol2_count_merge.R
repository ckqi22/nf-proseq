#!/usr/bin/env Rscript
# =============================================================================
# pol2_count_merge.R — Merge per-sample POL2_COUNT single-base gene-span count
# files into one count matrix.
#
...
#
#   - sample name is derived from the filename (strip ".pol2.counts.txt")
#   - 'length' is carried from the first file (annotation-derived, identical
#     across samples since every sample uses the same gene.bed)
# =============================================================================
...
# ── Parse args ──
argv <- arg_parser("Merge per-sample Pol II count files into a matrix")
...
read_counts <- function(f) {
    ...
    sample <- sub("\\.pol2\\.counts\\.txt$", "", basename(f))
...
main <- function(argv) {
    ...
    if (length(files) == 0) stop("[pol2_count_merge] no input count files given")
    for (f in files) if (!file.exists(f)) stop("[pol2_count_merge] count file not found: ", f)
    ...
    # gene.bed, so gene sets are identical in practice).
    ...
    message("[pol2_count_merge] ", nrow(res), " genes x ", ncol(res),
            " cols -> ", argv$output)
}
